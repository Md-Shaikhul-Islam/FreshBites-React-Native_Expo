...
function convertDBToAppProduct(dbProduct: DBProduct): Product {
  return {
    ...
    creatorName: dbProduct.creator_name,
    creatorEmail: dbProduct.creator_email,
  };
}
...
class DataManager {
  ...
  // Add new item
  async addItem(item: Product, isPremium: boolean, user?: User): Promise<boolean> {
    try {
      ...
      if (this.useDatabase) {
        ...
        // Use user ID if provided, otherwise fall back to device ID for backward compatibility
        if (user?.id) {
          await addProductToDB(dbProduct, user.id);
        } else {
  ...
  // Delete item
  async deleteItem(itemId: string, isPremium: boolean, user?: User): Promise<boolean> {
    try {
      ...
      if (this.useDatabase) {
        // Use user ID if provided, otherwise fall back to device ID for backward compatibility
        if (user?.id) {
          await deleteProductFromDB(itemId, user.id);
        } else {
          // Fallback to device ID for backward compatibility
          const { deviceId } = await getDeviceId();
