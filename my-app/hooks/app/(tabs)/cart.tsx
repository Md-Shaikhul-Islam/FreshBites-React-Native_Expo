...
export default function CartScreen() {
  ...
  const { isPremium, sandboxMode, user } = useUser();
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setTimeout(async () => {
        // Create orders in database (this triggers notifications to managers)
        if (user) {
          console.log('📦 Creating orders for user:', user.name);
          console.log('📦 Cart items:', items);
          
          const orderItems = items.map(item => {
            // Use priceValue (number) instead of parsing price (string)
            const price = item.priceValue || parseFloat(item.price) || 0;
            console.log(`📝 Item: ${item.title}, Price: ${price}, Quantity: ${item.quantity}`);
          ...
          const result = await createOrders(orderItems, user.id, user.name);
          
          if (!result.success) {
            setProcessing(false);
            Alert.alert('Order Failed', result.error || 'Failed to create order');
            return;
          }
          
          console.log('✅ Orders created:', result.orderIds);
        } else {
          console.warn('⚠️ No user found, order not saved to database');
        }
