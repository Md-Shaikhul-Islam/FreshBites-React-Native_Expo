...
export default function ManagerScreen() {
  ...
  const handleAddItem = async () => {
    ...
    try {
      const success = await dataManager.addItem(newItem, isPremiumItem, user || undefined);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const success = await dataManager.deleteItem(itemId, isPremium, user || undefined);
                ...
                {allProducts.map((item) => (
                  <View key={item.id} style={[styles.itemCard, { backgroundColor: colors.card }]}>
                    <View style={{ flex: 1 }}>
                      ...
                      {item.creatorName && (
                        <ThemedText style={{ fontSize: 11, opacity: 0.5, marginTop: 4 }}>
